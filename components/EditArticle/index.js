...
import {
  postArticleUrl, getQiniuTokenUrl
} from '../../config';
...
import {
  POST_ARTICLE_TYPE, POST_ARTICLE_COPY, UPLOAD_MAX_SIZE
...
class EditArticle extends Component {
  ...
  componentWillMount() {
    ...
    const {
      title, short, type, url, content, articleID = '', createTime
    } = dataSource;
  ...
  async beforeUpload(file) {
    const { password } = sessionStorage;
    let {
      bucket, imageTwo, markdownUploadLink, radioValue
    } = this.state;
    ...
    const newFileName = `image/${imageTwo || 'common'}/${font}_${lastModified}_${size}_${+new Date()}${back}`;
    const config = {
      useCdnDomain: true,
      region: null
    };
    ...
    const qiniu_upload_link = bucket_domin[bucket] + newFileName;
    this.setState({
  ...
  render() {
    ...
    // 上传
    const {
      previewVisible, previewImage, fileList, radioValue
    } = this.state;
    const uploadButton = (
      <div>
        <Icon type="plus"/>
        ...
        {markdownUploadLink.length
          ? markdownUploadLink.map((v, i) => (
            <Button
              ...
              onClick={this.onCopyLink.bind(this, v)}
              type="danger"
              size="small"
            >
              第
