...
import {
  getAdminBlogList, getIpList, getSearchList, postComments, postUserComments
} from "../../store/actions";
import {
  getAdminBlogUrl, getBlogUrl, getIpUrl, postCommentUrl, postUserCommentUrl
} from "../../config";
...
export const REQ_ACTION = {
  ...
  scrollBTMLoading(_this) {
    const { dispatch } = _this.props;
    let { pageSize: num } = _this.state;
    const { tabKey } = _this.state;
    const { password } = sessionStorage;
    const queryStringObj = {
      type: ALL,
      num,
    ...
    const footerDom = document.getElementsByClassName('footer-content')[0];
    const { innerHeight: windowHeight } = window;
    const { bottom } = footerDom.getBoundingClientRect();
    if (bottom - windowHeight < 1 && tabKey !== '1') {
      ...
      const newQueryStringObj = { ...queryStringObj, num: newNum };
      if (tabKey === '4') {
        // 浏览记录分页
