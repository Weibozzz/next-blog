...
export const TABLE_DATA = {
  ...
  articleUserCommentData(getUserCommentsData, commentsUserData, _this) {
    ...
    const columnsUserComments = keysUserComments
        && keysUserComments[0] ? keysUserComments[0].map(v => {
        return SET_TABLE.setUserCommentWidth(v, _this);
      }) : [];
    const dataCommentsUserData = commentsUserData.map(
      (v, i) => Object.assign({}, v, { key: i }, { createTime: formatTime(v.createTime) })
    );
  ...
  articleComment(getCommentsData, _this) {
    ...
    const columnsAdminComments = keysAdminComments
    && keysAdminComments[0] ? keysAdminComments[0].map(v => {
  ...
  getView({ date }) {
    if (date) {
      ...
      const { t_view: curView } = viewListData[0];
      const { t_view: yesView } = viewListData[1];
